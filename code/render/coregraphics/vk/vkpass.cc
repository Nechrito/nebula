...
void SetupPass(const PassId pid)
{
    ...
    images.Resize(loadInfo.colorAttachments.Size() + (loadInfo.depthStencilAttachment != RenderTextureId::Invalid() ? 1 : 0));
    ...
    for (i = 0; i < loadInfo.colorAttachments.Size(); i++)
    {
        ...
        layers = Math::n_max(layers, (uint32_t)RenderTextureGetNumLayers(loadInfo.colorAttachments[i]));
    ...
    for (i = 0; i < loadInfo.subpasses.Size(); i++)
    {
        ...
        {
            n_assert(loadInfo.depthStencilAttachment != RenderTextureId::Invalid());
    ...
    for (i = 0; i < loadInfo.colorAttachments.Size(); i++)
    {
        VkFormat fmt = VkTypes::AsVkFramebufferFormat(RenderTextureGetPixelFormat(loadInfo.colorAttachments[i]));
        ...
        attachment.samples = RenderTextureGetMSAA(loadInfo.colorAttachments[i]) ? VK_SAMPLE_COUNT_16_BIT : VK_SAMPLE_COUNT_1_BIT;
    }

    // use depth stencil attachments if pointer is not null
    if (loadInfo.depthStencilAttachment != CoreGraphics::RenderTextureId::Invalid())
    {
        VkAttachmentDescription& attachment = attachments[i];
        ...
        attachment.format = VkTypes::AsVkFramebufferFormat(RenderTextureGetPixelFormat(loadInfo.depthStencilAttachment));
        attachment.loadOp = loadOps[loadIdx];
        attachment.storeOp = storeOps[storeIdx];
        attachment.stencilLoadOp = loadOps[stencilLoadIdx];
        attachment.stencilStoreOp = storeOps[stencilStoreIdx];
        attachment.samples = RenderTextureGetMSAA(loadInfo.depthStencilAttachment) ? VK_SAMPLE_COUNT_16_BIT : VK_SAMPLE_COUNT_1_BIT;
        numUsedAttachments++;
    }
    ...
    if (loadInfo.depthStencilAttachment != CoreGraphics::RenderTextureId::Invalid())
    {
        images[i] = RenderTextureGetVkAttachmentImageView(loadInfo.depthStencilAttachment);
        const CoreGraphics::TextureDimensions dims = RenderTextureGetDimensions(loadInfo.depthStencilAttachment);
        VkRect2D& rect = loadInfo.rects[i];
        rect.offset.x = 0;
        rect.offset.y = 0;
        rect.extent.width = dims.width;
...
CreatePass(const PassCreateInfo& info)
{
	...
	SizeT numImages = info.colorAttachments.Size() + (info.depthStencilAttachment != RenderTextureId::Invalid() ? 1 : 0);
...
PassWindowResizeCallback(const PassId id)
{
    ...
    if (loadInfo.depthStencilAttachment != RenderTextureId::Invalid())
    {
        // FIXME: depth/stencil attachment gets incorrect size
