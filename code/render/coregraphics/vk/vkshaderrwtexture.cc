...
*/
void
SetupShaderRWTexture(const ShaderRWTextureId id, ShaderRWTextureInfo const& adjustedInfo)
{
    ...
    CoreGraphicsImageLayout& layout = shaderRWTextureAllocator.Get<2>(id.id24);
    ...
    VkImageViewCreateInfo viewCreate =
    {
        ...
        0,
        loadInfo.img,
        adjustedInfo.layers > 1 ? VK_IMAGE_VIEW_TYPE_2D_ARRAY : VK_IMAGE_VIEW_TYPE_2D,
        vkformat,
    ...
    VkUtilities::ImageBarrier(CoreGraphics::SubmissionContextGetCmdBuffer(sub),
        ...
        VkUtilities::ImageMemoryBarrier(loadInfo.img, viewRange, VK_ACCESS_HOST_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VkTypes::AsVkImageLayout(layout)));

    if (adjustedInfo.registerBindless)
        if (adjustedInfo.layers > 1)
            runtimeInfo.bind = VkShaderServer::Instance()->RegisterTexture(id, Texture2DArray);
        else
            runtimeInfo.bind = VkShaderServer::Instance()->RegisterTexture(id, Texture2D);
    else
        runtimeInfo.bind = 0; // use placeholder


    // store RW texture info for use when resizing
    shaderRWTextureAllocator.Get<3>(id.id24) = adjustedInfo;
}
...
CreateShaderRWTexture(const ShaderRWTextureCreateInfo& info)
{
	...
	SetupShaderRWTexture(ret, adjustedInfo);
...
*/
void
CleanupShaderRWTexture(const ShaderRWTextureId id)
{
    VkShaderRWTextureLoadInfo& loadInfo = shaderRWTextureAllocator.Get<0>(id.id24);
    VkShaderRWTextureRuntimeInfo& runtimeInfo = shaderRWTextureAllocator.Get<1>(id.id24);
...
{
    ShaderRWTextureInfo adjustedInfo = shaderRWTextureAllocator.Get<3>(id.id24);
    ShaderRWTextureInfoResizeHelper(adjustedInfo, info);

    CleanupShaderRWTexture(id);
    SetupShaderRWTexture(id, adjustedInfo);

    CoreGraphics::RegisterShaderRWTexture(adjustedInfo.name, id);
}
...
{
    const VkShaderRWTextureLoadInfo& loadInfo = shaderRWTextureAllocator.Get<0>(id.id24);
    ShaderRWTextureResizeInfo info;
